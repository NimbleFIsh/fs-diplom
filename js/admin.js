...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const inpListener = e => {
        network({ 'command': 'getHoll', 'holl': e.target.value.substr(4) }, d => {
            ...
            if (e.target.offsetParent.offsetParent.id == 'holls') {
            ...
            } else if (e.target.offsetParent.offsetParent.id == 'holls-price') {
    ...
    const insertMenu = data => {
        ...
        get('conf-step__selectors-box', false, true).forEach((el, i) => {
            el.getElementsByTagName('input')[0].checked = true;
            const h = el.getElementsByTagName('input')[0].value.substr(4);
            
        //    const timeline = document.createElement('div');
        //     timeline.classList.add('conf-step__seances-timeline');
        //     get('seasons', true).insertAdjacentHTML('beforeEnd', 
        //         `<div class="conf-step__seances-hall"><h3 class="conf-step__seances-title">Зал ${h}</h3></div>`);
                
            network({ 'command': 'getHoll', 'holl': h }, d => {
                const data = JSON.parse(d);
                configurePoles[0].placeholder = data.size[0];
                configurePoles[1].placeholder = data.size[1];
                configurePoles[2].placeholder = data.price.standart;
                ...
                configurePoles[3].value = '';
                reRenderHoll(data.size[0], data.size[1], data.place);
                // if (data.seasons.length > 0) data.seasons.forEach(e => timeline.insertAdjacentHTML('beforeEnd', 
                // `<div class="conf-step__seances-movie" style="width: 60px; background-color: rgb(133, 255, 137); left: ${i*60}px;">
                //     <p class="conf-step__seances-movie-title">${e.film}</p>
                //     <p class="conf-step__seances-movie-start">${e.time}</p>
                // </div>`));                
            });

            // get('seasons', true).children[i].insertAdjacentElement('beforeEnd', timeline);
        });
    }
    ...
    get('conf-step__button-accent', false, true).forEach(el => el.addEventListener('click', e => { // Открытие модального окна или отправка формы
        if (get('addHole').classList.contains('active')) {
            ...
            network({ 'command': 'film', 'mode': 'a', 'name': data['name'], 'time': data['time'] }, res => addFilm(data));
            ...
            network({ 'command': 'season', 'holl': h.parentElement.previousElementSibling.innerText.substr(4), 'mode': 'r', 'film':
                get('removeSeason', true).getElementsByTagName('span')[0].innerText.split(' на время ')[0] }, () => h.remove());
        }
        else if (e.target.offsetParent.id) {
            let size = { 'r': parseInt(configurePoles[0].value), 'm': parseInt(configurePoles[1].value) };
            let cur = null;
            get('conf-step__radio', false, true).forEach(f => { if (f.checked ) cur = f; });
            ...
            get(e.target.dataset.id, true).classList.add('active'); // Показ модального окна
        }
    ...
    get('conf-step__seances-movie', false, true).forEach(el => el.addEventListener('click', e => {
    ...
    get('conf-step__seances-timeline', false, true).forEach(el => el.addEventListener('click', e => {
